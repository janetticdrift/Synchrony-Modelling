...
for (x in 1:length(env_condition)) {
  for (y in 1:length(beta_range)) {
    ...
    for (z in 1:runs) {
      ...
      beta_vector <- rnorm(species*species, mean=beta_range, sd=0.05) #Set sd = 0, species to 2
...
ggplot(M1, aes(x=Var1, y=Var2, fill=value)) + 
  geom_tile() + 
  scale_fill_gradient2(low="#008080", high ="#ca562c", mid = "#f6edbd", midpoint = 1, limit = c(0,6)) +
