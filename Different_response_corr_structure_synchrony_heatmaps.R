...
runs <- 100 # number of runs was 2000
...
env_condition_s2 <- seq(-.25, .25, .01)
env_condition_s1 <- c(seq(.25, 0, -.01), seq(.01, 0.25, .01))

#Set outputs of interest
cv_total_biomass <-matrix(NA,nrow=length(env_condition_s1), ncol=length(beta_range))
VR <- matrix(NA,nrow=length(env_condition_s1), ncol=length(beta_range))


for (x in 1:length(env_condition_s1)) {
  for (y in 1:length(beta_range)) {
    ...
    for (z in 1:runs) {
      ...
      sigmaE1 <- -env_condition_s1[x] #env effect on species 1
      sigmaE2 <- -env_condition_s2[x]  #env effect on species 2
  ...
  print(x/length(env_condition_s1))
}
...
pdf(file="/Users/lash1937/Dropbox/Computer Documents/Janette Davidson/Modeling manuscript/Final_neg_pos_cov_s2.pdf",
    height=4.5, width=6)
ggplot(M1, aes(x=Var1, y=Var2, fill=value)) + 
  geom_tile() + 
  scale_fill_gradient2(low="#008080", high ="#ca562c", mid = "#f6edbd", midpoint = 1, limit = c(0,2)) +
  labs(x= expression(paste("Effect of Environmental Variability (", sigma[E2],")")), y= expression(paste("Strength of Competititon (", beta, ")")), title = "", fill="VR") +
  scale_x_continuous(breaks = seq(1, 51, 5), labels = c("-.25", "-.2", "-.15", "-.1", "-.05",
                                                       "0", "0.05", "0.1", "0.15", "0.2", "0.25"),
    sec.axis= dup_axis(breaks = seq(1, 51, 5), labels = c(".25", ".2", ".15", ".1", ".05",
                                                        "0", "0.05", "0.1", "0.15", "0.2", "0.25"),
    name=expression(paste("Effect of Environmental Variability (", sigma[E1],")")))) +
 # scale_x_continuous(breaks = seq(1, 51, 5), labels = c(".25", ".2", ".15", ".1", ".05",
 #                                                       "0", "0.05", "0.1", "0.15", "0.2", "0.25"),
 #   sec.axis= dup_axis(breaks = seq(1, 51, 5), labels = c("-.25", "-.2", "-.15", "-.1", "-.05",
 #                                                      "0", "0.05", "0.1", "0.15", "0.2", "0.25"),
...
###Figure 2: Resident communities' starting variance ratios----
time <- 200 
burn_in <- 100
runs <- 2000
...
for (x in 1:length(env_condition)) {
  for (y in 1:length(beta_range)) {
    ...
    for (z in 1:runs) {
      ...
      beta12 <- beta   #effect of species 2 on species 1
      beta21 <- beta   #effect of species 1 on species 2
      sigmaE2 <- -env_condition[x] #env effect on species 1
      sigmaE1 <- -.25  #env effect on species 2
      miuE <- rnorm(time, mean = 0, sd = 1) #environmental timeseries variation
      miuD1 <- rnorm(time, mean = 0, sd = 1) #dem timeseries for species 1
      miuD2 <- rnorm(time, mean = 0, sd = 1) #dem timeseries for species 2
...
quartz(width=6, height=5)
pdf(file="/Users/lash1937/Dropbox/Computer Documents/Janette Davidson/Modeling manuscript/Uncorrelated_s2.pdf",
    height=4.5, width=6)
ggplot(M1, aes(x=Var1, y=Var2, fill=value)) + 
  geom_tile() + 
  scale_fill_gradient2(low="#008080", high ="#ca562c", mid = "#f6edbd", midpoint = 1, limit = c(0,2)) +
  ...
  scale_x_continuous(breaks = seq(1, 26, 5), labels = c("0", "0.05", "0.1", "0.15", "0.2", "0.25"),
                     sec.axis= dup_axis(breaks = seq(1, 26, 5), labels = c("0.25", "0.25", "0.25", "0.25", "0.25", "0.25"),
                                        name=expression(paste("Effect of Environmental Variability (", sigma[E1],")")))) +
  #scale_x_continuous(breaks = seq(1, 26, 5), labels = c("0.25", "0.25", "0.25", "0.25", "0.25", "0.25"),
  #                   sec.axis= dup_axis(breaks = seq(1, 26, 5), labels = c("0", "0.05", "0.1", "0.15", "0.2", "0.25"),
  #                                      name=expression(paste("Effect of Environmental Variability (", sigma[E1],")")))) +
  scale_y_continuous(breaks = seq(0, 20, 5), labels = c("0", "0.2", "0.45", "0.7", "0.95")) +
  theme(axis.title=element_text(size=14), #change axis title size
        axis.text=element_text(size=12), #change axid text size
